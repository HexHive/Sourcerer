...
def compile(compiler, compileArgs, allocOption, baseOption, castOption):
    ...
    # OLDER COMPILATION TASK - KEPT FOR REFERENCE
    # args  = [compiler, "-O0", "-std=c++11", "-fsanitize=typeplus",  "firstmodule.cpp", "typecheck.cpp", "allocate.cpp", "secondmodule.cpp", "-DALLOC_" + allocOption, "-DBASE_" + baseOption, "-DCAST_" + castOption]
    # args.extend(compileArgs)
    # subprocess.call(args);d

    args  = [compiler, "-O0", "firstmodule.cpp", "typecheck.cpp", "allocate.cpp", "secondmodule.cpp", "-DALLOC_" + allocOption, "-DBASE_" + baseOption, "-DCAST_" + castOption]
    args += ["-stdlib=libc++", "-nostdinc++", "-Wl,-rpath,/workspace/typesafety-vtable/LLVM/build/lib", "-L/workspace/typesafety-vtableLLVM/build/lib"]
    args += ["-I/workspace/typesafety-vtable/LLVM/build/include/c++/v1", "-fsanitize=typeplus", "-flto", "-fvisibility=hidden", "-mllvm", "-apply-vtable-standard"]
    args += ["-mllvm", "-check-base-to-derived-casting", "-mllvm", "-check-unrelated-casting"]
    args.extend(compileArgs)
...
allocOptions = ["STACK", "STACK_ARRAY", "STACK_ARRAY_DEEP", "MALLOC", "MALLOC_ARRAY", "MALLOC_VLA", "MALLOC_NO_INIT", "MALLOC_NO_INIT_REINTER", "CALLOC_ARRAY", "CALLOC_VLA", "REALLOC", "REALLOC_ARRAY", "REALLOC_VLA", "NEW", "NEW_ARRAY", "NEW_VLA", "OVERLOADED_NEW", "OVERLOADED_NEW_ARRAY", "OVERLOADED_NEW_VLA", "GLOBAL", "GLOBAL_ARRAY", "GLOBAL_ARRAY_DEEP", "ARGUMENT"]
