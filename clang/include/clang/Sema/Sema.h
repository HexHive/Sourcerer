...
private:
  ...
  void CheckVptrInsertorCall(FunctionDecl *FDecl, QualType ThisType,
...
public:
  ...
  static void printCtor(StringRef message, const CXXRecordDecl* Record, bool debug = false, bool dump=false, CXXMethodDecl* target=nullptr) {
    if(debug) {
      llvm::errs() << "  " << message << " ";
      if(target) {
        target->printQualifiedName(llvm::errs());
      } else {
        Record->printQualifiedName(llvm::errs());
      }
      llvm::errs()  << "\n";
      if(debug && dump) {
        std::string n = "\t"; // names 
        std::string v = "\t"; // values
        n.append("tp ");
        v.append(std::to_string(Record->isTemplateDecl())).append("  ");
        n.append("tp ");
        v.append(std::to_string(Record->isTemplated())).append("  ");
        n.append("td ");
        v.append(std::to_string(Record->getTemplateDepth())).append("  ");
        n.append("Po ");
        v.append(std::to_string(Record->isPolymorphic())).append("  ");
        n.append("Fo ");
        v.append(std::to_string(Record->isForcePoly())).append("  ");
        n.append("SC ");
        v.append(std::to_string(Record->isStruct() || Record->isClass())).append("  ");
        n.append("!B ");
        v.append(std::to_string(!Record->isBeingDefined())).append("  ");
        n.append("!DT ");
        v.append(std::to_string(!Record->isDependentType())).append("   ");
        n.append("NI ");
        v.append(std::to_string(Record->needsImplicitDefaultConstructor())).append("  ");
        n.append("!ha ");
        v.append(std::to_string(!Record->hasDefaultConstructor())).append("   ");
        if(target) {
          n.append("Im ");
          v.append(std::to_string(target->isImplicit())).append("  ");
          n.append("Dc ");
          if(CXXConstructorDecl* constructor = dyn_cast<CXXConstructorDecl>(target)) {
            v.append(std::to_string(constructor->isDefaultConstructor())).append("  ");
          } else {
            v.append("   "); 
          }
          n.append("Dd ");
          v.append(std::to_string(target->isDefaulted())).append("  ");
          n.append("Us ");
          v.append(std::to_string(target->isUsed())).append("  ");
          n.append("Db ");
          v.append(std::to_string(target->doesThisDeclarationHaveABody())).append("  ");
          n.append("De ");
          v.append(std::to_string(target->isDeleted())).append("  ");
          n.append("Bo ");
          v.append(std::to_string(target->willHaveBody())).append("  ");
          n.append("In ");
          v.append(std::to_string(target->isInvalidDecl())).append("  ");
          n.append("Vi ");
          v.append(std::to_string(target->getVisibility())).append("  ");
          n.append("Wb ");
          v.append(std::to_string(target->willHaveBody())).append("  ");
          n.append("Ed ");
          v.append(std::to_string(target->isExplicitlyDefaulted())).append("  ");
          n.append("Tr ");
          v.append(std::to_string(target->isTrivial())).append("  ");
          n.append("FD ");
          v.append(std::to_string(target->isFirstDecl())).append("  ");
          n.append("DD ");
          v.append(std::to_string(target->isThisDeclarationADefinition())).append("  ");
          n.append("RE ");
          v.append(std::to_string(target->isThisDeclarationReferenced())).append("  ");
          n.append("RE ");
          v.append(std::to_string(target->isReferenced())).append("  ");
          n.append("TD ");
          v.append(std::to_string(target->isTemplateDecl())).append("  ");
        target->dumpColor();
        }
        llvm::errs() << n << "\n"; 
        llvm::errs() << v << "\n"; 
...
public:
  ...
  VptrInsertorDecl *
  findInheritingVptrInsertor(SourceLocation Loc, VptrInsertorDecl *BaseCtor,
                            ConstructorUsingShadowDecl *DerivedShadow);
  ...
  ExprResult
  BuildVptrInsertorExpr(SourceLocation ConstructLoc, QualType DeclInitType,
                        VptrInsertorDecl *Constructor, bool Elidable,
                        MultiExprArg Exprs,
  ...
  BuildVptrInsertorExpr(SourceLocation ConstructLoc, QualType DeclInitType,
                        NamedDecl *FoundDecl,
                        VptrInsertorDecl *Constructor, MultiExprArg Exprs,
                        bool HadMultipleCandidates, bool IsListInitialization,
                        bool IsStdInitListInitialization,
                        bool RequiresZeroInit, CXXConstructionKind ConstructKind,
                        SourceRange ParenRange);
  ...
  VptrInsertorDecl *DeclareImplicitDefaultVptrInsertor(
  ...
  void DefineVptrInsertor(SourceLocation CurrentLocation,
  ...
  bool CompleteVptrInsertorCall(VptrInsertorDecl *Constructor,
                               QualType DeclInitType, MultiExprArg ArgsPtr,
                               SourceLocation Loc,
                               SmallVectorImpl<Expr *> &ConvertedArgs,
                               bool AllowExplicit = false,
  ...
  bool SetVptrInsertorInitializers(
      VptrInsertorDecl *Constructor, bool AnyErrors,
      ArrayRef<CXXCtorInitializer *> Initializers = std::nullopt);
  ...
  void TypePPMarkBaseAndMemberDestructorsReferenced(SourceLocation Loc,
                                              CXXRecordDecl *Record);
  ...
  void MarkVTableUsed(SourceLocation Loc, CXXRecordDecl *Class,
                      bool DefinitionRequired = false, bool vptrDefine = false);
...
public:
  ...
  ExprResult TypeppBuildCXXDefaultInitExpr(SourceLocation Loc, FieldDecl *Field);
...
public:
  ...
  DeclContextLookupResult LookupVptrInsertors(CXXRecordDecl *Class);

  /// Look up the default constructor for the given class.
  CXXConstructorDecl *LookupDefaultConstructor(CXXRecordDecl *Class);
  VptrInsertorDecl *LookupVptrInsertor(CXXRecordDecl *Class);
