...
class VptrInsertorCall : public AnyCXXConstructorCall {
  friend class CallEventManager;
...
protected:
  ...
  /// \param St The path-sensitive state at this point in the program.
  /// \param LCtx The location context at this point in the program.
  VptrInsertorCall(const VptrInsertorExpr *CE, const MemRegion *Target,
                     ProgramStateRef St, const LocationContext *LCtx,
                     CFGBlock::ConstCFGElementRef ElemRef)
      : AnyCXXConstructorCall(CE, Target, St, LCtx, ElemRef) {}

  VptrInsertorCall(const VptrInsertorCall &Other) = default;

  void cloneTo(void *Dest) const override { new (Dest) VptrInsertorCall(*this); }

public:
  const VptrInsertorExpr *getOriginExpr() const override {
    return cast<VptrInsertorExpr>(AnyFunctionCall::getOriginExpr());
  }

  const VptrInsertorDecl *getDecl() const override {
    return getOriginExpr()->getConstructor();
  }
  ...
  Kind getKind() const override { return CE_VptrInsertor; }
  StringRef getKindAsString() const override { return "CXXConstructorCall"; }

  static bool classof(const CallEvent *CA) {
    return CA->getKind() == CE_VptrInsertor;
  }
};
...
public:
  ...
  CallEventRef<VptrInsertorCall>
  getVptrInsertorCall(const VptrInsertorExpr *E, const MemRegion *Target,
                        ProgramStateRef State, const LocationContext *LCtx,
                        CFGBlock::ConstCFGElementRef ElemRef) {
    return create<VptrInsertorCall>(E, Target, State, LCtx, ElemRef);
  }
