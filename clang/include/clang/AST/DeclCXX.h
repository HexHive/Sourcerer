...
public:
  ...
  bool needsVptrInitializer() const {
    if(data().Polymorphic)
      return true;
    if(field_empty())
      return false;
    for(auto *Field : fields())
      if(Field->getType()->getAsCXXRecordDecl())
        if(Field->getType()->getAsCXXRecordDecl()->needsVptrInitializer())
          return true;
    return false;
  }
...
class InheritedVptrInsertor {
  ConstructorUsingShadowDecl *Shadow = nullptr;
  VptrInsertorDecl *BaseCtor = nullptr;

public:
  InheritedVptrInsertor() = default;
  InheritedVptrInsertor(ConstructorUsingShadowDecl *Shadow,
  ...
  VptrInsertorDecl *getConstructor() const { return BaseCtor; }
};
...
class VptrInsertorDecl final
    : public CXXMethodDecl,
      private llvm::TrailingObjects<VptrInsertorDecl, InheritedVptrInsertor,
                                    ExplicitSpecifier> {
  public:
  LazyCXXCtorInitializersPtr CtorInitializers;

  VptrInsertorDecl(ASTContext &C, CXXRecordDecl *RD, SourceLocation StartLoc,
                     const DeclarationNameInfo &NameInfo, QualType T,
                     TypeSourceInfo *TInfo, ExplicitSpecifier ES,
                     bool UsesFPIntrin, bool isInline,
                     bool isImplicitlyDeclared, ConstexprSpecKind ConstexprKind,
                     InheritedVptrInsertor Inherited,
                     Expr *TrailingRequiresClause);

  void anchor() override;

  size_t numTrailingObjects(OverloadToken<InheritedVptrInsertor>) const {
    return VptrInsertorDeclBits.IsInheritingConstructor;
  }
  size_t numTrailingObjects(OverloadToken<ExplicitSpecifier>) const {
    return VptrInsertorDeclBits.HasTrailingExplicitSpecifier;
  }
  ExplicitSpecifier getExplicitSpecifierInternal() const {
    if (VptrInsertorDeclBits.HasTrailingExplicitSpecifier)
      return *getTrailingObjects<ExplicitSpecifier>();
    return ExplicitSpecifier(
        nullptr, VptrInsertorDeclBits.IsSimpleExplicit
  ...
  uint64_t getTrailingAllocKind() const {
    return numTrailingObjects(OverloadToken<InheritedVptrInsertor>()) |
...
public:
  ...
  static VptrInsertorDecl *CreateDeserialized(ASTContext &C, GlobalDeclID ID,
                                                uint64_t AllocKind);
  static VptrInsertorDecl *
  Create(ASTContext &C, CXXRecordDecl *RD, SourceLocation StartLoc,
         const DeclarationNameInfo &NameInfo, QualType T, TypeSourceInfo *TInfo,
         ExplicitSpecifier ES, bool UsesFPIntrin, bool isInline, bool isImplicitlyDeclared,
         ConstexprSpecKind ConstexprKind,
         InheritedVptrInsertor Inherited = InheritedVptrInsertor(),
         Expr *TrailingRequiresClause = nullptr);

  void setExplicitSpecifier(ExplicitSpecifier ES) {
    assert((!ES.getExpr() ||
            VptrInsertorDeclBits.HasTrailingExplicitSpecifier) &&
           "cannot set this explicit specifier. no trail-allocated space for "
           "explicit");
    if (ES.getExpr())
      *getCanonicalDecl()->getTrailingObjects<ExplicitSpecifier>() = ES;
    else
      VptrInsertorDeclBits.IsSimpleExplicit = ES.isExplicit();
  }
  ...
  unsigned getNumCtorInitializers() const {
      return VptrInsertorDeclBits.NumCtorInitializers;
  }

  void setNumCtorInitializers(unsigned numCtorInitializers) {
    VptrInsertorDeclBits.NumCtorInitializers = numCtorInitializers;
    // This assert added because NumCtorInitializers is stored
    // in VptrInsertorDeclBits as a bitfield and its width has
    // been shrunk from 32 bits to fit into VptrInsertorDeclBitfields.
    assert(VptrInsertorDeclBits.NumCtorInitializers ==
  ...
  VptrInsertorDecl *getTargetConstructor() const;
  ...
  void setInheritingConstructor(bool isIC = true) {
    VptrInsertorDeclBits.IsInheritingConstructor = isIC;
  }

  /// Get the constructor that this inheriting constructor is based on.
  InheritedVptrInsertor getInheritedVptrInsertor() const {
    return isInheritingConstructor() ?
      *getTrailingObjects<InheritedVptrInsertor>() : InheritedVptrInsertor();
  }

  VptrInsertorDecl *getCanonicalDecl() override {
    return cast<VptrInsertorDecl>(FunctionDecl::getCanonicalDecl());
  }
  const VptrInsertorDecl *getCanonicalDecl() const {
    return const_cast<VptrInsertorDecl*>(this)->getCanonicalDecl();
  }

  // Implement isa/cast/dyncast/etc.
  static bool classof(const Decl *D) { return classofKind(D->getKind()); }
  static bool classofKind(Kind K) { return K == VptrInsertor; }
};
