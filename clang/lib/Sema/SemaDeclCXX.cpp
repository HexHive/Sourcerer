...
namespace {
  class UninitializedFieldVisitor
    ...
    const VptrInsertorDecl *VptrInsertor;
  ...
  public:
    ...
    void CheckInitializerVptr(Expr *E, const VptrInsertorDecl *FieldConstructor,
  ...
  static void DiagnoseUninitializedFieldsVptr(
      Sema &SemaRef, const VptrInsertorDecl *Constructor) {
    ...
    for (const auto *FieldInit : Constructor->inits()) {
      ...
      if (CXXDefaultInitExpr *Default =
        ...
        if (!InitExpr)
          continue;
        // In class initializers will point to the constructor.
        UninitializedChecker.CheckInitializerVptr(InitExpr, Constructor,
                                              FieldInit->getAnyMember(),
                                              FieldInit->getBaseClass());
      } else {
        UninitializedChecker.CheckInitializerVptr(InitExpr, nullptr,
                                              FieldInit->getAnyMember(),
                                              FieldInit->getBaseClass());
      }
    }
...
static bool
TypePPBuildImplicitBaseInitializer(Sema &SemaRef, VptrInsertorDecl *Constructor,
  ...
  case IIK_Vptr: {
    InitializationKind InitKind
      = InitializationKind::CreateVptr(Constructor->getLocation());
    InitializationSequence InitSeq(SemaRef, InitEntity, InitKind, std::nullopt);
    if(InitSeq.Failed())
    llvm::errs() << InitSeq.Failed() << " oh no\n";
  ...
  if (BaseInit.isInvalid()) {
    llvm::errs() << "BaseInit is invalid\n";
  }

  BaseInit = SemaRef.MaybeCreateExprWithCleanups(BaseInit);
  if (BaseInit.isInvalid()) {
    llvm::errs() << "BaseInit is invalid2\n";
    return true;
  }
...
static bool
BuildImplicitMemberInitializer(Sema &SemaRef, VptrInsertorDecl *Constructor,
                               ImplicitInitializerKind ImplicitInitKind,
  ...
  assert((ImplicitInitKind == IIK_Vptr) &&
  ...
  if (FieldBaseElementType->isRecordType()) {
    InitializedEntity InitEntity =
        Indirect ? InitializedEntity::InitializeMember(Indirect, nullptr,
                 ...
                 : InitializedEntity::InitializeMember(Field, nullptr,
                                                       /*Implicit*/ true);
    InitializationKind InitKind =
      InitializationKind::CreateVptr(Loc);
    ...
    if (MemberInit.isInvalid()){
      llvm::errs() << InitSeq.Failed() << "\n";
      llvm::errs() << MemberInit.isInvalid() << "\n";
      llvm::errs() << "invalid2]\n";
      return true;
    }
    //MemberInit = SemaRef.MaybeCreateExprWithCleanups(MemberInit);
...
namespace {
struct TypePPBaseAndFieldInfo {
  Sema &S;
  VptrInsertorDecl *Ctor;
  ...
  TypePPBaseAndFieldInfo(Sema &S, VptrInsertorDecl *Ctor, bool ErrorsInInits)
...
static bool CollectFieldInitializer(Sema &SemaRef, TypePPBaseAndFieldInfo &Info,
                                    FieldDecl *Field,
                                    IndirectFieldDecl *Indirect = nullptr) {
  if (Field->isInvalidDecl())
    return false;

  if (auto x = Field->getType()->getAsCXXRecordDecl()) {
    if (!x->needsVptrInitializer()) 
      return false;
  } else {
  ...
  if (Field->hasInClassInitializer() && !Info.isImplicitCopyOrMove()) {
    ExprResult DIE =
        SemaRef.TypeppBuildCXXDefaultInitExpr(Info.Ctor->getLocation(), Field);
    if (DIE.isInvalid())
      return true;
...
bool Sema::SetVptrInsertorInitializers(VptrInsertorDecl *Constructor, bool AnyErrors,
                               ArrayRef<CXXCtorInitializer *> Initializers) {
  if (Constructor->isDependentContext()) {
    // Just store the initializers as written, they will be checked during
    // instantiation.
  ...
  TypePPBaseAndFieldInfo Info(*this, Constructor, AnyErrors);
  ...
  for (auto &VBase : ClassDecl->vbases()) {
    if (CXXCtorInitializer *Value
      ...
      bool IsInheritedVirtualBase = !DirectVBases.count(&VBase);
      CXXCtorInitializer *CXXBaseInit;
      if (TypePPBuildImplicitBaseInitializer(*this, Constructor, Info.IIK,
                                       &VBase, IsInheritedVirtualBase,
                                       CXXBaseInit)) {
        HadError = true;
        continue;
  ...
  if (NumInitializers > 0) {
    ...
    // Constructors implicitly reference the base and member
    // destructors.
    TypePPMarkBaseAndMemberDestructorsReferenced(Constructor->getLocation(),
                                           Constructor->getParent());
  }
...
void
Sema::TypePPMarkBaseAndMemberDestructorsReferenced(SourceLocation Location,
                                             CXXRecordDecl *ClassDecl) {
...
Sema::getDefaultedFunctionKind(const FunctionDecl *FD) {
  if (auto *MD = dyn_cast<CXXMethodDecl>(FD)) {
    ...
    if (const VptrInsertorDecl *Ctor = dyn_cast<VptrInsertorDecl>(FD)) {
      return CXXSpecialMemberKind::VptrInsertor;
    }
...
public:
  ...
  std::pair<VptrInsertorDecl *, bool>
  findVptrInsertorForBase(CXXRecordDecl *Base, VptrInsertorDecl *Ctor) const {
    auto It = InheritedFromBases.find(Base->getCanonicalDecl());
    if (It == InheritedFromBases.end())
      return std::make_pair(nullptr, false);
    ...
    if (It->second)
      return std::make_pair(
          S.findInheritingVptrInsertor(UseLoc, Ctor, It->second),
          It->second->constructsVirtualBase());
...
static bool
TypePPspecialMemberIsConstexpr(Sema &S, CXXRecordDecl *ClassDecl,
                         CXXSpecialMemberKind CSM, unsigned Quals,
                         bool ConstRHS,
                         VptrInsertorDecl *InheritedCtor = nullptr,
                         Sema::InheritedConstructorInfo *Inherited = nullptr) {
  // If we're inheriting a constructor, see if we need to call it for this base
  // class.
  if (InheritedCtor) {
    assert(CSM == CXXSpecialMemberKind::DefaultConstructor);
    auto BaseCtor =
        Inherited->findVptrInsertorForBase(ClassDecl, InheritedCtor).first;
    if (BaseCtor)
      return BaseCtor->isConstexpr();
  }
...
static bool TypePPdefaultedSpecialMemberIsConstexpr(
    Sema &S, CXXRecordDecl *ClassDecl, CXXSpecialMemberKind CSM,
    bool ConstArg, VptrInsertorDecl *InheritedCtor = nullptr,
    Sema::InheritedConstructorInfo *Inherited = nullptr) {
  if (!S.getLangOpts().CPlusPlus11)
    return false;
  ...
  if (!S.getLangOpts().CPlusPlus23) {
    for (const auto &B : ClassDecl->bases()) {
      ...
      if (!BaseType)
        continue;
      CXXRecordDecl *BaseClassDecl = cast<CXXRecordDecl>(BaseType->getDecl());
      if (!TypePPspecialMemberIsConstexpr(S, BaseClassDecl, CSM, 0, ConstArg,
                                    InheritedCtor, Inherited))
        return false;
    }
  }
...
VptrInsertorDecl *Sema::DeclareImplicitDefaultVptrInsertor(
                                                     CXXRecordDecl *ClassDecl) {
  // C++ [class.ctor]p5:
  //   A default constructor for a class X is a constructor of class X
  //   that can be called without an argument. If there is no
  ...
  DeclaringSpecialMember DSM(*this, ClassDecl,
                             CXXSpecialMemberKind::VptrInsertor);
  if (DSM.isAlreadyBeingDeclared())
    return nullptr;

  bool Constexpr = TypePPdefaultedSpecialMemberIsConstexpr(
      *this, ClassDecl, CXXSpecialMemberKind::VptrInsertor, false);
  ...
  SourceLocation ClassLoc = ClassDecl->getLocation();
  DeclarationName Name
    = Context.DeclarationNames.getVptrInsertorName(ClassType);
  DeclarationNameInfo NameInfo(Name, ClassLoc);
  VptrInsertorDecl *DefaultCon = VptrInsertorDecl::Create(
      Context, ClassDecl, ClassLoc, NameInfo, /*Type*/ QualType(),
      /*TInfo=*/nullptr, ExplicitSpecifier(),
      getCurFPFeatures().isFPConstrained(),
      /*isInline=*/true, /*isImplicitlyDeclared=*/true,
  ...
  if (getLangOpts().CUDA)
    CUDA().inferTargetForImplicitSpecialMember(
        ClassDecl, CXXSpecialMemberKind::VptrInsertor, DefaultCon,
        /* ConstRHS */ false,
        /* Diagnose */ false);

  // We don't need to use SpecialMemberIsTrivial here; triviality for default
  // constructors is easy to compute.
  DefaultCon->setTrivial(true);
...
void Sema::DefineVptrInsertor(SourceLocation CurrentLocation,
                        VptrInsertorDecl *Constructor) {
  //assert((Constructor->isDefaulted() && Constructor->isDefaultConstructor() &&
  //        !Constructor->doesThisDeclarationHaveABody() &&
  //        !Constructor->isDeleted()) &&
  //  "DefineImplicitDefaultConstructor - call it for implicit default ctor");
  ...
  ResolveExceptionSpec(CurrentLocation,
                       Constructor->getType()->castAs<FunctionProtoType>());
  if (!isa<ClassTemplateSpecializationDecl>(ClassDecl)) {
    MarkVTableUsed(CurrentLocation, ClassDecl, false, true);
  } else {
    if (Constructor->getQualifiedNameAsString().find("MappingQ1<3") ==
        std::string::npos) {
      if (Constructor->getQualifiedNameAsString().find("MappingQ1<2,") ==
          std::string::npos) {
        if (Constructor->getQualifiedNameAsString().find("MappingQ1<1, 2") ==
            std::string::npos) {
          if (Constructor->getQualifiedNameAsString().find("MappingQ1<1, 3") ==
              std::string::npos) {
            if (Constructor->getQualifiedNameAsString().find("Tensor") ==
                std::string::npos) {
              if (Constructor->getQualifiedNameAsString().find(
                      "Triangulation") == std::string::npos) {
                if (Constructor->getQualifiedNameAsString().find("DoF") ==
                    std::string::npos) {
                  MarkVTableUsed(CurrentLocation, ClassDecl, false, true);
                  //llvm::errs()
  ...
  if (SetVptrInsertorInitializers(Constructor, /*AnyErrors=*/false)) {
    Constructor->setInvalidDecl();
    return;
  }
  ...
  DiagnoseUninitializedFieldsVptr(*this, Constructor);
...
VptrInsertorDecl *
Sema::findInheritingVptrInsertor(SourceLocation Loc,
                                VptrInsertorDecl *BaseCtor,
                                ConstructorUsingShadowDecl *Shadow) {
  ...
  for (NamedDecl *Ctor : Derived->lookup(Name))
    if (declaresSameEntity(cast<VptrInsertorDecl>(Ctor)
                               ->getInheritedVptrInsertor()
                               .getConstructor(),
                           BaseCtor))
      return cast<VptrInsertorDecl>(Ctor);
  ...
  bool Constexpr = BaseCtor->isConstexpr() &&
                   TypePPdefaultedSpecialMemberIsConstexpr(
                       *this, Derived, CXXSpecialMemberKind::DefaultConstructor,
                       false, BaseCtor, &ICI);

  VptrInsertorDecl *DerivedCtor = VptrInsertorDecl::Create(
      ...
      clang::InheritedVptrInsertor(Shadow, BaseCtor),
...
ExprResult
Sema::BuildVptrInsertorExpr(SourceLocation ConstructLoc, QualType DeclInitType,
                            NamedDecl *FoundDecl,
                            VptrInsertorDecl *Constructor,
  ...
  return BuildVptrInsertorExpr(ConstructLoc, DeclInitType,
...
Sema::BuildVptrInsertorExpr(SourceLocation ConstructLoc, QualType DeclInitType,
  ...
  if (auto *Shadow = dyn_cast<ConstructorUsingShadowDecl>(FoundDecl)) {
    Constructor = findInheritingVptrInsertor(ConstructLoc, Constructor, Shadow);
    if (DiagnoseUseOfDecl(Constructor, ConstructLoc))
      return ExprError();
  }

  return BuildVptrInsertorExpr(
...
/// BuilVptrInsertortExpr - Creates a complete call to a constructor,
...
bool Sema::CompleteVptrInsertorCall(VptrInsertorDecl *Constructor,
  ...
  CheckVptrInsertorCall(Constructor, DeclInitType,
...
void Sema::MarkVTableUsed(SourceLocation Loc, CXXRecordDecl *Class,
                          bool DefinitionRequired, bool VptrDefine) {
