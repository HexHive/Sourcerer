    ...
    if (OldMethod && NewMethod) {
      ...
      if (!isFriend && NewMethod->getLexicalDeclContext()->isRecord() &&
        ...
        if (!inTemplateInstantiation()) {
          ...
          else if (isa<VptrInsertorDecl>(OldMethod))
            NewDiag = diag::err_vptr_insertor_redeclared;
...
bool Sema::CheckFunctionDeclaration(Scope *S, FunctionDecl *NewFD,
  ...
  if (!getLangOpts().CPlusPlus14 && MD && MD->isConstexpr() &&
      !MD->isStatic() && !isa<CXXConstructorDecl>(MD) && !isa<VptrInsertorDecl>(MD) &&
...

  {
    ...
    if (FD) {
      ...
      if (!FD->isInvalidDecl()) {
        ...
        else if (VptrInsertorDecl *Destructor = dyn_cast<VptrInsertorDecl>(FD))
...
void Sema::ActOnTagFinishDefinition(Scope *S, Decl *TagD,
  ...
  if (isa<CXXRecordDecl>(TagD)) {
    ...
    if (
        getLangOpts().Sanitize.has(SanitizerKind::TypePlus) && llvm::ClVtableStandard) {
      if (Record && Record->isPolymorphic() && (Record->isStruct() || Record->isClass()) &&
          !Record->isBeingDefined()) {
        std::list<VptrInsertorDecl *> vptrInsertor = {};
        for (auto *M : Record->methods()) {
          if (VptrInsertorDecl *target = dyn_cast<VptrInsertorDecl>(M)) {
            vptrInsertor.push_back(target);
            break;
          }
        }
        if (debug) {
          llvm::errs() << Record->getNameAsString() << "\n";
          llvm::errs() << "# vptr insert: " << vptrInsertor.size() << "\n";
        }
        if (!Record->isDependentType()) {
          printCtor("Entry Non Template constructor instantiation: ", Record,
                    debug, true);
          if (vptrInsertor.empty()) { // no default const
            VptrInsertorDecl *target =
                DeclareImplicitDefaultVptrInsertor(Record);
            vptrInsertor.push_back(target);
            printCtor("Non Template: declared", Record, debug, true,
                      target);
          }
          if (vptrInsertor.size() == 1) {
            VptrInsertorDecl *target = vptrInsertor.front();
            printCtor("Found 1 def cons. ", Record, debug, true, target);
            target->dropAttr<ExcludeFromExplicitInstantiationAttr>();
            if (target->isDeleted() ||
                (target->isImplicit() && !target->willHaveBody() &&
                 !target->hasBody())) {
              printCtor("Defined temp 1: ", Record, debug, true, target);
              target->setDeletedAsWritten(false);
              DefineVptrInsertor(Record->getLocation(),
                                                     target);
            }
            MarkFunctionReferenced(Record->getLocation(), target);
            target->setIsUsed();
            target->markUsed(Context);
            printCtor("Should be good: ", Record, debug, true, target);
          } else {
            llvm::errs() << "UNEXPECTED Multiple default const\n";
          }
        } else {
          if(debug) {
            llvm::errs() << "Actually, we are dependnat: ";
