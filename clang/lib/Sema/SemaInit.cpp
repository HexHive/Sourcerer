...
void InitializationSequence::AddVptrInsertorInitializationStep(
    DeclAccessPair FoundDecl, VptrInsertorDecl *Constructor, QualType T,
...
static OverloadingResult ResolveConstructorOverload(
  ...
  for (NamedDecl *D : Ctors) {
    ...
    if((!Info.Constructor || Info.Constructor->isInvalidDecl()) && (!Info.VptrInsertor || Info.VptrInsertor->isInvalidDecl())) {
      continue;
    }
    ...
    if (Info.ConstructorTmpl)
      ...
      S.AddOverloadCandidate(Info.Constructor, Info.FoundDecl, Args,
        ...
        S.AddOverloadCandidate(Info.VptrInsertor, Info.FoundDecl, Args,
                             CandidateSet, SuppressUserConversions,
                             /*PartialOverloading=*/false, AllowExplicit,
                             AllowExplicitConv);
      } 
...
static void TryVptrInsertorInitialization(Sema &S,
                                         const InitializedEntity &Entity,
                                         const InitializationKind &Kind,
                                         MultiExprArg Args, QualType DestType,
                                         QualType DestArrayType,
  ...
  //     applicable constructors are enumerated, and the best one is chosen
  //     through overload resolution.
  DeclContext::lookup_result Ctors = S.LookupVptrInsertors(DestRecordDecl);
  ...
  if (IsListInit) {
    ...
    // If the initializer list has no elements and T has a default constructor,
    // the first phase is omitted.
    if (!(UnwrappedArgs.empty() && S.LookupVptrInsertor(DestRecordDecl)))
      Result = ResolveConstructorOverload(
          S, Kind.getLocation(), Args, CandidateSet, DestType, Ctors, Best,
          CopyInitialization, AllowExplicit,
          /*OnlyListConstructors=*/true, IsListInit, RequireActualConstructor);
  ...
  VptrInsertorDecl *CtorDecl = cast<VptrInsertorDecl>(Best->Function);
  if (Result != OR_Deleted) {
    // C++11 [dcl.init]p6:
    //   If a program calls for the default initialization of an object
    //   of a const-qualified type T, T shall be a class type with a
  ...
  // Add the constructor initialization step. Any cv-qualification conversion is
  // subsumed by the initialization.
  Sequence.AddVptrInsertorInitializationStep(
      Best->FoundDecl, CtorDecl, DestArrayType, HadMultipleCandidates,
      IsListInit | IsInitListCopy, AsInitializerList);
}
...
static void TryDefaultVptrInsertorInitialization(Sema &S,
                                     const InitializedEntity &Entity,
                                     const InitializationKind &Kind,
                                     InitializationSequence &Sequence) {
  assert(Kind.getKind() == InitializationKind::IK_Vptr);
  ...
  if (DestType->isRecordType() && S.getLangOpts().CPlusPlus) {
    TryVptrInsertorInitialization(S, Entity, Kind, std::nullopt, DestType,
...
InitializationSequence::InitializationSequence(
    Sema &S, const InitializedEntity &Entity, const InitializationKind &Kind,
    MultiExprArg Args, bool TopLevelOfInitList, bool TreatUnavailableAsInvalid, int vptrInsertor)
    : FailedOverloadResult(OR_Success),
      FailedCandidateSet(Kind.getLocation(), OverloadCandidateSet::CSK_Normal) {
  if(Kind.getKind() == InitializationKind::IK_Vptr)
    InitializeVptrInsertorFrom(S, Entity, Kind, Args, TopLevelOfInitList,
                   TreatUnavailableAsInvalid); 
  else
...
void InitializationSequence::InitializeVptrInsertorFrom(Sema &S,
                                            const InitializedEntity &Entity,
                                            const InitializationKind &Kind,
                                            MultiExprArg Args,
                                            bool TopLevelOfInitList,
  ...
  // Handle default initialization.
  if (Kind.getKind() == InitializationKind::IK_Vptr) {
    TryDefaultVptrInsertorInitialization(S, Entity, Kind, *this);
    return;
  }
  ...
  if (DestType->isRecordType()) {
    ...
    //       source type is the same class as, or a derived class of, the
    //       class of the destination, constructors are considered. [...]
    if (Kind.getKind() == InitializationKind::IK_Direct || Kind.getKind() == InitializationKind::IK_Vptr ||
        (Kind.getKind() == InitializationKind::IK_Copy &&
         (Context.hasSameUnqualifiedType(SourceType, DestType) ||
          (Initializer && S.IsDerivedFrom(Initializer->getBeginLoc(),
                                          SourceType, DestType))))) {
      TryVptrInsertorInitialization(S, Entity, Kind, Args, DestType, DestType,
                                   *this);
...
static ExprResult
PerformVptrInsertorInitialization(Sema &S,
                                 const InitializedEntity &Entity,
                                 const InitializationKind &Kind,
                                 MultiExprArg Args,
                                 const InitializationSequence::Step& Step,
  ...
  VptrInsertorDecl *Constructor
    = cast<VptrInsertorDecl>(Step.Function.Function);
  bool HadMultipleCandidates = Step.Function.HadMultipleCandidates;
  ...
  if (Kind.getKind() == InitializationKind::IK_Default) {
    ...
    if (Constructor->isDefaulted() && Constructor->isDefaultConstructor() &&
        Constructor->isTrivial() && !Constructor->isUsed(false)) {
      S.runWithSufficientStackSpace(Loc, [&] {
        S.DefineVptrInsertor(Loc, Constructor); });
    }
  }
  ...
  // Determine the arguments required to actually perform the constructor
  // call.
  if (S.CompleteVptrInsertorCall(Constructor, Step.Type, Args, Loc,
                                ConstructorArgs, AllowExplicitConv,
                                IsListInitialization))
    return ExprError();
  ...
  if (isExplicitTemporary(Entity, Kind, NumArgs)) {
    ...
    VptrInsertorDecl *CalleeDecl = Constructor;
    if (auto *Shadow = dyn_cast<ConstructorUsingShadowDecl>(
            Step.Function.FoundDecl.getDecl())) {
      CalleeDecl = S.findInheritingVptrInsertor(Loc, Constructor, Shadow);
    }
    S.MarkFunctionReferenced(Loc, CalleeDecl);

    llvm::errs() << "Would call temporary\n";
    //CurInit = S.CheckForImmediateInvocation(
    //    CXXTemporaryObjectExpr::Create(
    //        S.Context, CalleeDecl,
    //        Entity.getType().getNonLValueExprType(S.Context), TSInfo,
    ...
    // unconditionally.
    if (Entity.allowsNRVO())
      CurInit = S.BuildVptrInsertorExpr(Loc, Step.Type,
                                        Step.Function.FoundDecl,
                                        Constructor, /*Elidable=*/true,
                                        ConstructorArgs,
                                        HadMultipleCandidates,
...
ExprResult InitializationSequence::Perform(Sema &S,
  ...
  for (step_iterator Step = step_begin(), StepEnd = step_end();
    ...
    case SK_ConstructorInitializationFromList: {
      ...
      if (isa<VptrInsertorDecl>(Step->Function.Function))
        CurInit = PerformVptrInsertorInitialization(S, UseTemporary ? TempEntity :
                                                                   Entity,
                                                 Kind, Arg, *Step,
                                               ConstructorInitRequiresZeroInit,
    ...
    case SK_StdInitializerListConstructorCall: {
      ...
      if (isa<VptrInsertorDecl>(Step->Function.Function)) {
        ...
        CurInit = PerformVptrInsertorInitialization(
    ...
    switch (FailedOverloadResult) {
      ...
      case OR_No_Viable_Function:
        ...
        if (Kind.getKind() == InitializationKind::IK_Default &&
            (Entity.getKind() == InitializedEntity::EK_Base ||
             Entity.getKind() == InitializedEntity::EK_Member) &&
            isa<VptrInsertorDecl>(S.CurContext)) {
          // This is implicit default initialization of a member or
          // base within a constructor. If no viable function was
          // found, notify the user that they need to explicitly
          // initialize this base/member.
          VptrInsertorDecl *Constructor
            = cast<VptrInsertorDecl>(S.CurContext);
          const CXXRecordDecl *InheritedFrom = nullptr;
          if (auto Inherited = Constructor->getInheritedVptrInsertor())
            InheritedFrom = Inherited.getShadowDecl()->getNominatedBaseClass();
          if (Entity.getKind() == InitializedEntity::EK_Base) {
            // Disabling error for non-initialized member (e.g., const issue)
            if (!S.getLangOpts().Sanitize.has(SanitizerKind::TypePlus)) {
