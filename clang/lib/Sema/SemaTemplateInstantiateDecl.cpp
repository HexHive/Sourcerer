...
Decl *TemplateDeclInstantiator::VisitCXXMethodDecl(
  ...
  if ((cast<CXXRecordDecl>(D->getParent())->isLambda() &&
      !D->getTypeSourceInfo() &&
      isa<CXXConstructorDecl, CXXDestructorDecl>(D)) || isa<VptrInsertorDecl>(D)) {
    TypeSourceInfo *TSI =
  ...
  if (CXXConstructorDecl *Constructor = dyn_cast<CXXConstructorDecl>(D)) {
   ...
   } else if (VptrInsertorDecl *Constructor = dyn_cast<VptrInsertorDecl>(D)) {
    Method = VptrInsertorDecl::Create(
        SemaRef.Context, Record, StartLoc, NameInfo, T, TInfo,
        InstantiatedExplicitSpecifier, Constructor->UsesFPIntrin(),
        Constructor->isInlineSpecified(), false,
        Constructor->getConstexprKind(), InheritedVptrInsertor(),
        TrailingRequiresClause);
    Method->setRangeEnd(Constructor->getEndLoc());
...
Decl *TemplateDeclInstantiator::VisitVptrInsertorDecl(VptrInsertorDecl *D) {
