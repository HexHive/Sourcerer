...
Sema::InstantiateClass(SourceLocation PointOfInstantiation,
  ...
  if (getLangOpts().Sanitize.has(SanitizerKind::TypePlus) && llvm::ClVtableStandard) {
    if (Instantiation && (Instantiation->isStruct() || Instantiation->isClass()) && !Instantiation->isBeingDefined()) {
      ...
      std::list<VptrInsertorDecl*> defaultConst = {};
      std::list<VptrInsertorDecl*> defaultedConst = {};
      for (CXXMethodDecl *MD : llvm::make_range(Instantiation->method_begin(), Instantiation->method_end())) {
      ...
      if(!Instantiation->isDependentType()) {
        ...
        if(defaultConst.empty()) { // no default const
          VptrInsertorDecl* target = DeclareImplicitDefaultVptrInsertor(Instantiation);
          defaultConst.push_back(target);
          printCtor("Template: declared", Instantiation, debug, true, target);
        } 
        if(defaultConst.size() == 1) {
          VptrInsertorDecl* target = defaultConst.front();
          ...
          if (target->isDeleted() || (target->isImplicit() && !target->willHaveBody() && !target->hasBody())) {
            ...
            // This is problematic as we will define a constructor which does not initialize all fields.
            // If the code later actually wants to define a default constructor, it will fails.
            // Possible fix: add a field in ConstructorDecl to indicate that it is a default constructor set by TypePP.
            // Change the code so that in case we try to reuse it, we erase (how?) the definition and redefine it.
            DefineVptrInsertor(Instantiation->getLocation(), target);
          }
          //MarkFunctionReferenced(Instantiation->getLocation(), target);
          //DiagnoseUseOfDecl(target, Instantiation->getLocation());
