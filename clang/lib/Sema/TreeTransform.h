...
#include "llvm/Frontend/OpenMP/OMP.inc"
  ...
  ExprResult RebuildVptrInsertorExpr(
      QualType T, SourceLocation Loc, VptrInsertorDecl *Constructor,
    ...
    VptrInsertorDecl *FoundCtor = Constructor;
    if (Constructor->isInheritingConstructor())
      FoundCtor = Constructor->getInheritedVptrInsertor().getConstructor();

    SmallVector<Expr *, 8> ConvertedArgs;
    if (getSema().CompleteVptrInsertorCall(FoundCtor, T, Args, Loc,
                                          ConvertedArgs))
      return ExprError();

    return getSema().BuildVptrInsertorExpr(Loc, T, Constructor,
                                           IsElidable,
                                           ConvertedArgs,
...
template<typename Derived>
ExprResult
TreeTransform<Derived>::TransformVptrInsertorExpr(VptrInsertorExpr *E) {
  ...
  VptrInsertorDecl *Constructor = cast_or_null<VptrInsertorDecl>(
  ...
  return getDerived().RebuildVptrInsertorExpr(
