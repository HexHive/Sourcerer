...
Sema::LookupSpecialMember(CXXRecordDecl *RD, CXXSpecialMemberKind SM,
  ...
  } else if (SM == CXXSpecialMemberKind::VptrInsertor) {
    Name = Context.DeclarationNames.getVptrInsertorName(CanTy);
    NumArgs = 0;
  } else {
  ...
  if (SM != CXXSpecialMemberKind::DefaultConstructor && SM != CXXSpecialMemberKind::VptrInsertor) {
    NumArgs = 1;
    Arg = &FakeArg;
  }
  ...
  if (R.empty()) {
    ...
    // destructor.
    assert((SM == CXXSpecialMemberKind::DefaultConstructor || SM == CXXSpecialMemberKind::VptrInsertor) &&
  ...
  for (NamedDecl *CandDecl : Candidates) {
    ...
    if (CXXMethodDecl *M = dyn_cast<CXXMethodDecl>(Cand->getUnderlyingDecl())) {
      if(isa<VptrInsertorDecl>(M)) {
        continue;
      } else if (SM == CXXSpecialMemberKind::CopyAssignment ||
          SM == CXXSpecialMemberKind::MoveAssignment)
      ...
      if(isa<VptrInsertorDecl>(CandDecl))
        continue;
      else if (SM == CXXSpecialMemberKind::CopyAssignment ||
          SM == CXXSpecialMemberKind::MoveAssignment)
...
VptrInsertorDecl *Sema::LookupVptrInsertor(CXXRecordDecl *Class) {
  SpecialMemberOverloadResult Result =
    LookupSpecialMember(Class, CXXSpecialMemberKind::VptrInsertor, false, false, false,
                        false, false);

  return cast_or_null<VptrInsertorDecl>(Result.getMethod());
}
...
DeclContext::lookup_result Sema::LookupVptrInsertors(CXXRecordDecl *Class) {
  // If the implicit constructors have not yet been declared, do so now.
  if (CanDeclareSpecialMemberFunction(Class)) {
    runWithSufficientStackSpace(Class->getLocation(), [&] {
      //NICOLAS BADOUX, actually could be only declared here.
      //if (Class->isForcePoly())
    });
  }

  CanQualType T = Context.getCanonicalType(Context.getTypeDeclType(Class));
  //IdentifierInfo& II = Context.Idents.get(Class->getNameAsString() + "_vptr_insertor");

  DeclarationName Name = Context.DeclarationNames.getVptrInsertorName(T);
  auto lk = Class->lookup(Name);
  if(lk.empty()) {
    auto target = DeclareImplicitDefaultVptrInsertor(Class);

              target->setDeletedAsWritten(false);
            MarkFunctionReferenced(Class->getLocation(), target);
              DefineVptrInsertor(Class->getLocation(),
                                        target);
    lk = Class->lookup(Name);
    if(lk.empty()) {
      llvm::errs() << "NOT FOUND again\n";
    }
  }
  return lk;
}
