...
ExprResult Sema::TypeppBuildCXXDefaultInitExpr(SourceLocation Loc, FieldDecl *Field) {
  assert(Field->hasInClassInitializer());
  ...
  //  }
  //}
  VptrInsertorDecl* vt = nullptr;
  if(ParentRD)
  for(CXXMethodDecl* M : ParentRD->methods()) {
    if(VptrInsertorDecl* vt2 = dyn_cast<VptrInsertorDecl>(M)) {
      vt = vt2;
      break;
    }
  }
  ...
  if(vt) {
    return VptrInsertorExpr::Create(Context, Field->getType(), Loc, vt,  false, MultiExprArg(), false, false, false, false, CXXConstructionKind::Complete, SourceRange());
  } else {
    return ExprError();
  }
  // DR1351:
...
void Sema::MarkFunctionReferenced(SourceLocation Loc, FunctionDecl *Func,
  ...
  // We are missing some consturctor otherwise, see test case about template NICOLAS TODO
