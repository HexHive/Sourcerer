...
static void TypePPEmitMemberInitializer(CodeGenFunction &CGF,
                                  const CXXRecordDecl *ClassDecl,
                                  CXXCtorInitializer *MemberInit,
                                  const VptrInsertorDecl *Constructor,
                                  FunctionArgList &Args) {
  //ApplyDebugLocation Loc(CGF, MemberInit->getSourceLocation());
  assert(MemberInit->isAnyMemberInitializer() &&
         "Must have member initializer!");
  ...
  if (Array && Constructor->isDefaulted() &&
    ...
    if (BaseElementTy.isPODType(CGF.getContext()) ||
        (CE && isMemcpyEquivalentSpecialMember(CE->getConstructor()))) {
      unsigned SrcArgIndex =
          CGF.CGM.getCXXABI().getSrcArgforCopyVptrInsertor(Constructor, Args);
      llvm::Value *SrcPtr
        = CGF.Builder.CreateLoad(CGF.GetAddrOfLocalVar(Args[SrcArgIndex]));
      LValue ThisRHSLV = CGF.MakeNaturalAlignAddrLValue(SrcPtr, RecordTy);
      LValue Src = CGF.EmitLValueForFieldInitialization(ThisRHSLV, Field);
  ...
  if (auto e = dyn_cast<clang::VptrInsertorExpr>(MemberInit->getInit())) {
    // if not a class, e->getConstructor will return the same type resulting in an ifinite loop
    if(e->getType()->isStructureOrClassType()){
      Constructor = e->getConstructor();
      Address addr = LHS.getAddress();
      //                     Address(ThisPtr,
      //                     CGF.getContext().getTypeAlignInChars(RecordTy));
      QualType type = CGF.getContext()
                          .getTypeDeclType(Constructor->getParent())
                          .getCanonicalType();
      if (type.getTypePtr() == ClassDecl->getTypeForDecl()) {
        //llvm::errs() << "Would habe been an error in function TypePPEmitMemberInitializer: infinite loop\n";
        return;
      }
      auto currAVS = AggValueSlot::forAddr(
          addr, type.getQualifiers(), AggValueSlot::IsDestructed,
          AggValueSlot::DoesNotNeedGCBarriers, AggValueSlot::IsNotAliased,
          AggValueSlot::DoesNotOverlap, AggValueSlot::IsNotZeroed,
          AggValueSlot::IsNotSanitizerChecked);
      CGF.EmitVptrInsertorCall(Constructor, Ctor_Complete,
                               /*ForVirtualBase=*/false,
                               /*Delegating=*/false, currAVS, e);
      // CGF.EmitExprAsInit(e, Field, LHS, false);
      // CGF.EmitInitializerForField(Field, LHS, MemberInit->getInit());
      }
  }
}
...
bool CodeGenFunction::IsVptrInsertorDelegationValid(
    const VptrInsertorDecl *Ctor) {
...
void CodeGenFunction::EmitAsanPrologueOrEpilogue(bool Prologue) {
  ...
  const CXXRecordDecl *ClassDecl =
      Prologue ? isa<CXXConstructorDecl>(CurGD.getDecl()) ? cast<CXXConstructorDecl>(CurGD.getDecl())->getParent() : cast<VptrInsertorDecl>(CurGD.getDecl())->getParent()
...
void CodeGenFunction::EmitVptrInsertorBody(FunctionArgList &Args) {
  EmitAsanPrologueOrEpilogue(true);
  const VptrInsertorDecl *Ctor = cast<VptrInsertorDecl>(CurGD.getDecl());
  ...
  // NICOLAS BADOUX TYPEPLUS, don't look up for other constructor in the case of vptr insertor
  //if (CtorType == Ctor_Complete && IsVptrInsertorDelegationValid(Ctor) &&
  //    CGM.getTarget().getCXXABI().hasConstructorVariants()) {
  // EmitDelegateVptrInsertorCall(Ctor, Ctor_Base, Args, Ctor->getEndLoc());
  //  llvm::errs() <<" would have been an exit: "  << "\n";
  //  return;
  //}
  ...
  // initializers.
  EmitVptrInsertorPrologue(Ctor, CtorType, Args);
...
namespace {
  ...
  class VptrInsertorMemcpyizer : public FieldMemcpyizer {
  private:
    /// Get source argument for copy constructor. Returns null if not a copy
    /// constructor.
    static const VarDecl *getTrivialCopySource(CodeGenFunction &CGF,
                                               const VptrInsertorDecl *CD,
                                               FunctionArgList &Args) {
      if (CD->isCopyOrMoveConstructor() && CD->isDefaulted())
        return Args[CGF.CGM.getCXXABI().getSrcArgforCopyVptrInsertor(CD, Args)];
      return nullptr;
    }
    ...
    bool isMemberInitMemcpyable(CXXCtorInitializer *MemberInit) const {
      ...
      QualType FieldType = Field->getType();
      VptrInsertorExpr *CE = dyn_cast<VptrInsertorExpr>(MemberInit->getInit());
  ...
  public:
    VptrInsertorMemcpyizer(CodeGenFunction &CGF, const VptrInsertorDecl *CD,
                          FunctionArgList &Args)
    ...
    void addMemberInitializer(CXXCtorInitializer *MemberInit) {
      if (isMemberInitMemcpyable(MemberInit)) {
        ...
        emitAggregatedInits();
        TypePPEmitMemberInitializer(CGF, ConstructorDecl->getParent(), MemberInit,
    ...
    void emitAggregatedInits() {
      if (AggregatedInits.size() <= 1) {
        ...
        if (!AggregatedInits.empty()) {
          ...
          TypePPEmitMemberInitializer(CGF, ConstructorDecl->getParent(),
  ...
  private:
    const VptrInsertorDecl *ConstructorDecl;
    bool MemcpyableCtor;
    FunctionArgList &Args;
...
void CodeGenFunction::EmitVptrInsertorPrologue(const VptrInsertorDecl *CD,
                                       CXXCtorType CtorType,
                                       FunctionArgList &Args) {
  if (CD->isDelegatingConstructor())
    return EmitDelegatingCXXVptrInsertorCall(CD, Args);

  const CXXRecordDecl *ClassDecl = CD->getParent();

  VptrInsertorDecl::init_const_iterator B = CD->init_begin(),
                                          E = CD->init_end();
  ...
  // And finally, initialize class members.
  FieldConstructionScope FCS(*this, LoadCXXThisAddress());
  VptrInsertorMemcpyizer CM(*this, CD, Args);
  for (; B != E; B++) {
    CXXCtorInitializer *Member = (*B);
    assert(!Member->isBaseInitializer());
    assert(Member->isAnyMemberInitializer() &&
...
void CodeGenFunction::EmitCXXAggrVptrInsertorCall(
    const VptrInsertorDecl *ctor, const ArrayType *arrayType,
    Address arrayBegin, const VptrInsertorExpr *E, bool NewPointerIsChecked,
    bool zeroInitialize) {
  QualType elementType;
  llvm::Value *numElements =
    emitArrayLength(arrayType, elementType, arrayBegin);

  EmitCXXAggrVptrInsertorCall(ctor, numElements, arrayBegin, E,
...
void CodeGenFunction::EmitCXXAggrVptrInsertorCall(const VptrInsertorDecl *ctor,
                                                 llvm::Value *numElements,
                                                 Address arrayBase,
                                                 const VptrInsertorExpr *E,
...

  {
    ...
    auto currAVS = AggValueSlot::forAddr(
        ...
        NewPointerIsChecked ? AggValueSlot::IsSanitizerChecked
                            : AggValueSlot::IsNotSanitizerChecked);
    EmitVptrInsertorCall(ctor, Ctor_Complete, /*ForVirtualBase=*/false,
                           /*Delegating=*/false, currAVS, E);
  }
...
void CodeGenFunction::EmitVptrInsertorCall(const VptrInsertorDecl *D,
                                             CXXCtorType Type,
                                             bool ForVirtualBase,
                                             bool Delegating,
  ...
  EmitVptrInsertorCall(D, Type, ForVirtualBase, Delegating, This, Args,
...
void CodeGenFunction::EmitCXXConstructorCall(const CXXConstructorDecl *D,
  ...
  if (CGM.getLangOpts().Sanitize.has(SanitizerKind::TypePlus) && llvm::ClTypePlusProfiling) {
    if(D->getParent() && D->getParent()->isForcePoly()) {
      llvm::Value* strPointer = Builder.CreateGlobalStringPtr(D->getParent()->getNameAsString());
      llvm::FunctionCallee ObjUpdateFunction = CGM.getModule().getOrInsertFunction("__profiling_constructor_init_call", llvm::FunctionType::get(VoidTy, {VoidPtrTy, Int8Ty}, false));
      Builder.CreateCall(ObjUpdateFunction, {strPointer, Builder.getInt8(1)});
    }
  }
...
static bool TypePPcanEmitDelegateCallArgs(CodeGenFunction &CGF,
                                    const VptrInsertorDecl *Ctor,
  ...
  if (CGF.getTarget().getCXXABI().areArgsDestroyedLeftToRightInCallee()) {
    ...
    const CGFunctionInfo &Info =
        CGF.CGM.getTypes().arrangeCXXVptrInsertorCall(Args, Ctor, Type, 0, 0);
...
void CodeGenFunction::EmitVptrInsertorCall(const VptrInsertorDecl *D,
  ...
  if (auto Inherited = D->getInheritedVptrInsertor()) {
    PassPrototypeArgs = getTypes().inheritingVptrInsertorHasParams(Inherited, Type);
    if (PassPrototypeArgs && !TypePPcanEmitDelegateCallArgs(*this, D, Type, Args)) {
  //    EmitInlinedInheritingCXXVptrInsertorCall(D, Type, ForVirtualBase,
  //                                            Delegating, Args);
      return;
    }
  }
  ...
  CGCXXABI::AddedStructorArgCounts ExtraArgs =
      CGM.getCXXABI().addImplicitVptrInsertorArgs(*this, D, Type, ForVirtualBase,
                                                 Delegating, Args);

  // Emit the call.
  llvm::Constant *CalleePtr = CGM.getAddrOfVptrInsertor(GlobalDecl(D, Type));
  const CGFunctionInfo &Info = CGM.getTypes().arrangeCXXVptrInsertorCall(
  ...
  if (CGM.getLangOpts().Sanitize.has(SanitizerKind::TypePlus) && llvm::ClTypePlusProfiling) {
    if(ClassDecl && ClassDecl->isForcePoly()) {
      llvm::Value* strPointer = Builder.CreateGlobalStringPtr(ClassDecl->getNameAsString());
      llvm::FunctionCallee ObjUpdateFunction = CGM.getModule().getOrInsertFunction("__profiling_vptr_insertor_call", llvm::FunctionType::get(VoidTy, {VoidPtrTy, Int8Ty}, false));
      Builder.CreateCall(ObjUpdateFunction, {strPointer, Builder.getInt8(1)});
    }
  }
...
void CodeGenFunction::EmitInlinedInheritingCXXVptrInsertorCall(
    const VptrInsertorDecl *Ctor, CXXCtorType CtorType, bool ForVirtualBase,
    bool Delegating, CallArgList &Args) {
  GlobalDecl GD(Ctor, CtorType);
  llvm::errs () << "Here in EmitInlinedInheritingCXXVptrInsertorCall\n";
  ...
  // Insert any ABI-specific implicit constructor arguments.
  CGM.getCXXABI().addImplicitVptrInsertorArgs(*this, Ctor, CtorType,
  ...
  EmitVptrInsertorPrologue(Ctor, CtorType, Params);
}
...
void
CodeGenFunction::EmitDelegateVptrInsertorCall(const VptrInsertorDecl *Ctor,
                                                CXXCtorType CtorType,
                                                const FunctionArgList &Args,
                                                SourceLocation Loc) {
  CallArgList DelegateArgs;
  llvm::errs() << "Here is EmitDelegateCXXConstructorCall\n";
  ...
  EmitVptrInsertorCall(Ctor, CtorType, /*ForVirtualBase=*/false,
...
void
CodeGenFunction::EmitDelegatingCXXVptrInsertorCall(const VptrInsertorDecl *Ctor,
                                                  const FunctionArgList &Args) {
  assert(Ctor->isDelegatingConstructor());
...
void CodeGenFunction::EmitVTablePtrCheckForCast(QualType T, Address Derived,
  ...
  if (!ClassDecl->isCompleteDefinition()) {
    ...
    llvm::Value *VTable;
    std::tie(VTable, ClassDecl) = CGM.getCXXABI().LoadVTablePtr(*this, Address(Derived), ClassDecl);
