...
bool CodeGenTypes::inheritingVptrInsertorHasParams(
    const InheritedVptrInsertor &Inherited, CXXCtorType Type) {
...
const CGFunctionInfo &
CodeGenTypes::arrangeVptrInsertorDeclaration(GlobalDecl GD) {
  ...
  if (auto *CD = dyn_cast<VptrInsertorDecl>(MD)) {
    // A base class inheriting constructor doesn't get forwarded arguments
    // needed to construct a virtual base (or base class thereof).
    if (auto Inherited = CD->getInheritedVptrInsertor())
      PassParams = inheritingVptrInsertorHasParams(Inherited, GD.getCtorType());
  }
...
const CGFunctionInfo &
CodeGenTypes::arrangeCXXVptrInsertorCall(const CallArgList &args,
