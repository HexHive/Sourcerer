...
public:
  ...
  size_t getSrcArgforCopyVptrInsertor(const VptrInsertorDecl *CD,
...
void MicrosoftCXXABI::EmitVptrInsertors(const VptrInsertorDecl *D) {
  ...
  if (D->hasAttr<DLLExportAttr>() && D->isDefaultConstructor() &&
    ...
    if (!hasDefaultCXXMethodCC(getContext(), D) || D->getNumParams() != 0) {
      assert(false && "vptr insertors should implement microsoft convention");
...
CGCXXABI::AddedStructorArgs MicrosoftCXXABI::getImplicitVptrInsertorArgs(
