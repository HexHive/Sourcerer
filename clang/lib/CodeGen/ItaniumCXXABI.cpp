...
public:
  ...
  void EmitVptrInsertors(const VptrInsertorDecl *D) override;
  ...
  AddedStructorArgs getImplicitVptrInsertorArgs(CodeGenFunction &CGF,
                                               const VptrInsertorDecl *D,
                                               CXXCtorType Type,
                                               bool ForVirtualBase,
  ...
  size_t getSrcArgforCopyVptrInsertor(const VptrInsertorDecl *,
...
void ItaniumCXXABI::EmitVptrInsertors(const VptrInsertorDecl *D) {
  // Just make sure we're in sync with TargetCXXABI.
  assert(CGM.getTarget().getCXXABI().hasConstructorVariants());
...
ItaniumCXXABI::buildStructorSignature(GlobalDecl GD,
  ...
  if ((isa<CXXConstructorDecl>(GD.getDecl()) ? GD.getCtorType() == Ctor_Base
                                             : isa<VptrInsertorDecl>(GD.getDecl()) ? GD.getVptrInsertorType() == Ctor_Base : GD.getDtorType() == Dtor_Base) &&
...
CGCXXABI::AddedStructorArgs ItaniumCXXABI::getImplicitVptrInsertorArgs(
    CodeGenFunction &CGF, const VptrInsertorDecl *D, CXXCtorType Type,
...
static StructorCodegen getCodegenToUse(CodeGenModule &CGM,
  ...
  } else if (const auto *DD = dyn_cast<VptrInsertorDecl>(MD)) {
    return StructorCodegen::Emit;
  } else {
...
void ItaniumCXXABI::emitCXXStructor(GlobalDecl GD) {
  ...
  const VptrInsertorDecl *VD = dyn_cast<VptrInsertorDecl>(MD);

  StructorCodegen CGType = getCodegenToUse(CGM, MD);

  if ((CD || VD) ? GD.getCtorType() == Ctor_Complete
