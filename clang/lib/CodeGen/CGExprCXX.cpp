...
void CodeGenFunction::InsertVptrForUnion(Address Addr, CXXRecordDecl* Parent, CXXRecordDecl* Child, Expr* E) {
  if (!llvm::ClVtableStandard &&
      CGM.getLangOpts().Sanitize.has(SanitizerKind::TypePlus))
    return;

  if (llvm::ClNoSetSourcererRTTI)
    return;
  ...
  // constructor call as it would reset all the fields already set if the type
  // was already correct.

  bool debug = false;
  // VTable: Just return when target object type is not appropriate
  if (Child->hasDefinition() && ((Child->isClass() || Child->isStruct())) && Child->isForcePoly()) {
    // VTable: Try to find insertor call
    for (CXXMethodDecl *MD : llvm::make_range(Child->method_begin(), Child->method_end())) {
      if (VptrInsertorDecl *target = dyn_cast<VptrInsertorDecl>(MD)) {
        ...
        const QualType &DestTy =
            Child->getTypeForDecl()->getCanonicalTypeInternal();
        VptrInsertorExpr *cee = VptrInsertorExpr::Create(
            getContext(), DestTy, E->getExprLoc(), target, false, Args,
            false, false, false, false, CXXConstructionKind::Complete,
            E->getSourceRange());
        QualType type = getContext().getTypeDeclType(target->getParent());
        ...
        if(debug) {
          llvm::errs() << "Inserting vptr for union: " << Parent->getName() << "\n";
        }
        EmitVptrInsertorCall(target, Ctor_Complete,
                             /*ForVirtualBase=*/false,
                             /*Delegating=*/false, currAVS, cee);
      }
    }
  }
}
...
RValue CodeGenFunction::EmitCXXMemberOrOperatorMemberCallExpr(
  ...
  if (const VptrInsertorDecl *Ctor = dyn_cast<VptrInsertorDecl>(MD)) {
...
void
CodeGenFunction::EmitVptrInsertorExpr(const VptrInsertorExpr *E,
                                      AggValueSlot Dest) {
  assert(!Dest.isIgnored() && "Must have a destination!");
  const VptrInsertorDecl *CD = E->getConstructor();
  ...
  if (const ArrayType *arrayType
    ...
    EmitCXXAggrVptrInsertorCall(CD, arrayType, Dest.getAddress(), E,
    ...
    case CXXConstructionKind::NonVirtualBase:
     ...
     EmitVptrInsertorCall(CD, Type, ForVirtualBase, Delegating, Dest, E);
  }
}
