    ...
    case CK_AddressSpaceConversion:
      if (auto PtrTy = CE->getSubExpr()->getType()->getAs<PointerType>()) {
        ...
        if ((CGF.SanOpts.has(SanitizerKind::CFIUnrelatedCast) ||
             (CGF.SanOpts.has(SanitizerKind::TypePlus) &&
              llvm::ClTypePlusCheckUnrelatedCasting)) &&
...
static void emitCheckHandlerCall(CodeGenFunction &CGF,
  ...
  if ((CGF.CGM.getLangOpts().Sanitize.has(SanitizerKind::TypePlus) &&
       (llvm::ClTypePlusCheckUnrelatedCasting ||
        llvm::ClTypePlusCheckDerivedCasting)) ||
      ((CGF.CGM.getLangOpts().Sanitize.has(SanitizerKind::CFIDerivedCast) ||
        CGF.CGM.getLangOpts().Sanitize.has(SanitizerKind::CFIUnrelatedCast)) &&
       llvm::ClTypePlusProfiling)) {
    // insert call to Typeplus.cpp check
    llvm::FunctionCallee ObjUpdateFunction =
        CGF.CGM.getModule().getOrInsertFunction(
            "__report_type_confusion",
            llvm::FunctionType::get(CGF.VoidTy,
                                    {CGF.Int8PtrTy, CGF.IntPtrTy, CGF.IntPtrTy},
                                    false));
    CGF.Builder.CreateCall(
        ObjUpdateFunction,
        {CGF.Builder.CreateBitCast(FnArgs[0], CGF.Int8PtrTy),
         CGF.Builder.CreateBitCast(FnArgs[1], CGF.IntPtrTy),
         CGF.Builder.CreateBitCast(FnArgs[2], CGF.IntPtrTy)});
  } else {
...
LValue CodeGenFunction::EmitMemberExpr(const MemberExpr *E) {
  ...
  if (E->isArrow()) {
    ...
    if (auto *Field = dyn_cast<FieldDecl>(E->getMemberDecl())) {
      CXXRecordDecl *Parent = dyn_cast<CXXRecordDecl>(Field->getParent());
      auto *type = Field->getType()
                       ->getPointeeOrArrayElementType()
                       ->getCanonicalTypeInternal()
                       .getTypePtrOrNull();

      // Check if the type is a C++ record type
      if (const auto *recordType = type->getAs<RecordType>()) {
        if (Parent && Parent->isUnion()) {
          if (CXXRecordDecl *cxxdecl = dyn_cast<CXXRecordDecl>(Parent)) {
            CXXRecordDecl *child =
                dyn_cast<CXXRecordDecl>(recordType->getDecl());
            if (child->isForcePoly()) {
              if (E->isArrow()) {
                InsertVptrForUnion(Addr, Parent, child, BaseExpr);
              }
            }
          }
        }
...
LValue CodeGenFunction::EmitCastLValue(const CastExpr *E) {
  ...
  case CK_BaseToDerived: {
    ...
    if (SanOpts.has(SanitizerKind::CFIDerivedCast) ||
        (SanOpts.has(SanitizerKind::TypePlus) &&
         llvm::ClTypePlusCheckDerivedCasting)) {
    ...
    llvm::HexTypeCommonUtil::logClassToFile(ConvertType(E->getSubExpr()->getType()), "/derived_casting_obj_init");
    llvm::HexTypeCommonUtil::logClassToFile(ConvertType(E->getType()), "/derived_casting_obj_init");
  ...
  case CK_LValueBitCast: {
    ...
    QualType DestTy = CE->getSubExpr()->getType();
    bool isDerivedCast = false;
    CXXRecordDecl* srcDecl, *destDecl;
    if(SrcTy->isAnyPointerType()) {
      srcDecl = SrcTy->getPointeeType()->getAsCXXRecordDecl();
    } else { 
    ...
    if (SanOpts.has(SanitizerKind::CFIUnrelatedCast) &&
      ...
      EmitVTablePtrCheckForCast(E->getType(), V,
                                /*MayBeNull=*/false,
                                isDerivedCast ? CFITCK_DerivedCast
...
LValue CodeGenFunction::EmitBinaryOperatorLValue(const BinaryOperator *E) {
  ...
  case TEK_Scalar: {
    ...
    if (CGM.getLangOpts().Sanitize.has(SanitizerKind::TypePlus) &&
        llvm::ClVtableStandard && !llvm::ClNoSetSourcererRTTI && E->isAssignmentOp()) {
              ...
              if (VptrInsertorDecl *target = dyn_cast<VptrInsertorDecl>(MD)) {
                if (target->isDefaultConstructor()) {
                  ...
                  for (uint i = 0; i < target->getNumParams(); i++) {
                    argsVec.emplace_back(
                        target->getParamDecl(i)->getDefaultArg());
                  }
                  ArrayRef<Expr *> Args = ArrayRef(argsVec);

                  // GlobalDecl GD(target, Ctor_Base);
                  // llvm::FunctionCallee initFunction =
                  // CGM.getAddrAndTypeOfCXXStructor(GD);
                  VptrInsertorExpr *cee = VptrInsertorExpr::Create(
                      getContext(),
                      DerivedClassDecl->getTypeForDecl()
                          ->getCanonicalTypeInternal(),
                      E->getExprLoc(), target, false, Args, false, false, false,
                      false, CXXConstructionKind::Complete,
                      E->getSourceRange());
                  ...
                  AggValueSlot avs = AggValueSlot::forAddr(
                      ...
                      AggValueSlot::DoesNotNeedGCBarriers,
                      AggValueSlot::IsAliased, AggValueSlot::DoesNotOverlap);
                  EmitVptrInsertorCall(target, Ctor_Base, false, false, avs,
                                       cee);
                  if (debug) {
                    llvm::errs() << "Found real union constructor "
