...
llvm::Function *CodeGenModule::codegenCXXStructor(GlobalDecl GD) {
  const CGFunctionInfo& FnInfo = isa<VptrInsertorDecl>(GD.getDecl()) ?
     getTypes().arrangeVptrInsertorDeclaration(GD) :
...
llvm::FunctionCallee CodeGenModule::getAddrAndTypeOfVptrInsertor(
    GlobalDecl GD, const CGFunctionInfo *FnInfo, llvm::FunctionType *FnType,
    bool DontDefer, ForDefinition_t IsForDefinition) {
  auto *MD = cast<CXXMethodDecl>(GD.getDecl());
  ...
  if (!FnType) {
    if (!FnInfo)
      FnInfo = &getTypes().arrangeVptrInsertorDeclaration(GD);
    FnType = getTypes().GetFunctionType(*FnInfo);
  }

  llvm::Constant *Ptr = GetOrCreateLLVMFunction(
      getMangledName(GD).str(), FnType, GD, /*ForVTable=*/false, DontDefer,
      /*IsThunk=*/false, /*ExtraAttrs=*/llvm::AttributeList(), IsForDefinition);
  return {FnType, Ptr};
}
