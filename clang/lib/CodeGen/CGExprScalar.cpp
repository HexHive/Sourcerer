...
public:
  ...
  bool isNotVoidCast(QualType CastSrcTy, QualType DestTy);
  bool isDerivedCast(CastExpr *E);
  void logBitCastClasses(CastExpr *E, CodeGenFunction* CGF);
...
bool CodeGenFunction::isPoolCustomAllocator(std::string fun_name, std::string class_name = "") {
  ...
  std::set<std::pair<std::string, std::string>> custom_allocator_class_functions_set= {
    ...
    std::make_pair("xalanc_1_10::MemoryManagerType", "allocate"), // actually a pool allocator for xalan 2017
    std::make_pair("MemoryManagerType", "allocate"), // actually a pool allocator for xalan 2017
    std::make_pair("class xercesc_2_7::MemoryManager", "allocate"), // actually a pool allocator for xalan 2017
    std::make_pair("MemoryManager", "allocate"), // actually a pool allocator for xalan 2006
    std::make_pair("class xercesc_2_5::MemoryManager *", "allocate"), // actually a pool allocator for xalan 2006
    std::make_pair("DOMDocumentImpl", "allocate"), // actually a pool allocator for xalan 2006
    std::make_pair("class xercesc_2_5::DOMDocumentImpl *", "allocate"), // actually a pool allocator for xalan 2006
    std::make_pair("class xercesc_2_5::DOMDocumentImpl", "allocate"), // actually a pool allocator for xalan 2006
    std::make_pair("class xercesc_2_7::DOMDocumentImpl", "allocate"), // actually a pool allocator for xalan 2006
  };
...
bool CodeGenFunction::isRealloc(std::string fun_name, std::string class_name = "") {
  std::set<std::pair<std::string, std::string>> custom_allocator_class_functions_set= {};
  std::set<std::string> custom_allocator_set { 
    "realloc",
    "pov_realloc", // POVRAY 2006 & 2017
...
AllocatorTypeAndMetaDataSize CodeGenFunction::isAllocatorCast(Stmt *CurStmt) {
  bool debug = false;
  AllocatorTypeAndMetaDataSize res;
  for (Stmt *Child : CurStmt->children()) {
      ...
      for (Stmt *subChild: Child->children()) {
        ...
        switch(subChild->getStmtClass()) {
          ...
          case Stmt::DeclRefExprClass:
            subExpr = dyn_cast<DeclRefExpr>(subChild);
            break;
          default:
            continue;
        }
    ...
    bool pool = CodeGenFunction::isPoolCustomAllocator(fun_name, class_name);
    bool custom = CodeGenFunction::isCustomAllocator(fun_name, class_name);
    bool realloc = CodeGenFunction::isRealloc(fun_name, class_name);
    int8_t header = CodeGenFunction::needsHeaderSpace(fun_name, class_name);
    if(debug && (pool || custom)) {
      llvm::errs() << "Found allocator " << pool << " " << custom << " " << class_name << (class_name != "" ?  "->" : "") << fun_name <<  " with header: " << std::to_string(header) << "\n";
    }
    if(pool) {
    ...
    if (custom) {
      if(realloc) {
        return std::make_pair(AllocatorType::REALLOC, header);
      }
...
bool isAllocCall(llvm::CallBase *val, const llvm::TargetLibraryInfo *tli) {
  ...
  return llvm::isAllocationFn(val, tli) &&
      (llvm::isMallocOrCallocLikeFn(val, tli) ||
...
std::pair<Value*, Value*> CodeGenFunction::numberOfConstructorCall(llvm::CallBase* callToMalloc, llvm::Type* allocatedType, AllocatorType allocatorType, int8_t custom_header_size) {
  ...
  llvm::FunctionCallee mallocUsableSize = M.getOrInsertFunction("malloc_usable_size", Builder.getInt32Ty(), Builder.getPtrTy());

  Value* alreadyAllocatedSpace = Builder.getInt32(0);

  if(allocatorType == AllocatorType::REALLOC) {
    ...
    Value* headerMetaDataLocation = Builder.CreateGEP(Builder.getInt8Ty(), *callToMalloc->arg_begin(), needHeaderSize);
  ...
  if(allocatorType == AllocatorType::ALLOCATOR || allocatorType == AllocatorType::REALLOC) {
    // Calling malloc_usable_size on the start of the allocated memory, we have to
    // remove the libc++ header
    Value* memoryWithHeaderRemoved = Builder.CreateGEP(Builder.getInt8Ty(), callToMalloc, Builder.getInt32(-custom_header_size));
  ...
  llvm::Value* nbr_obj = Builder.CreateSub(loopEndVal, loopStartVal);
  ...
  if(debug) {
    ...
    printArgs.push_back(allocatedSpaceWithoutCustomHeader);
    printArgs.push_back(loopEndVal);
    printArgs.push_back(nbr_obj);
  }
  ...
  if(allocatorType == AllocatorType::REALLOC) {
    ...
    nbr_obj = Builder.CreateSub(loopEndVal, loopStartVal);
    llvm::Value* isXPositive = Builder.CreateICmpSGT(nbr_obj, llvm::ConstantInt::get(nbr_obj->getType(), 0), "is_x_positive");

    // Step 2: Clamp x to 0 (max(0, x))
    nbr_obj = Builder.CreateSelect(isXPositive, nbr_obj, llvm::ConstantInt::get(nbr_obj->getType(), 0),"clamped_to_zero");
    printArgs.push_back(nbr_obj);
  }
  ...
  if(debug) {
    ...
    "\tAlready allocated: %d\n"
    "\tType size: %d\n"
    "\tNow allocated total: %d\n"
    "\tSpace allocated with header removed: %d\n"
    "Endval: %d\n"
    "nbr_obj: %d\n"
    "nbr_obj_clamped: %d\n";
    if(allocatorType == AllocatorType::REALLOC) {
  ...
  return std::make_pair(loopStartVal, nbr_obj);
}

bool ScalarExprEmitter::isNotVoidCast(QualType CastSrcTy, QualType DestTy) {
  return (CastSrcTy->isAnyPointerType() && (CastSrcTy->getPointeeType()->isStructureOrClassType() || CastSrcTy->getPointeeType()->isUnionType())) && (DestTy->isAnyPointerType() && (DestTy->getPointeeType()->isStructureOrClassType() || DestTy->getPointeeType()->isUnionType())); 
}

bool ScalarExprEmitter::isDerivedCast(CastExpr* CE) {
  QualType DestTy = CE->getType();
  QualType CastSrcTy = CE->getSubExpr()->getType();
  bool isNotVoidCast = ScalarExprEmitter::isNotVoidCast(CastSrcTy, DestTy);
  ...
  return isDerivedCast;
}

void ScalarExprEmitter::logBitCastClasses(CastExpr* CE, CodeGenFunction* CGF) {
  Expr *E = CE->getSubExpr();
  QualType DestTy = CE->getType();
  QualType CastSrcTy = E->getType();
  bool isDerivedCast = ScalarExprEmitter::isDerivedCast(CE);
  bool isNotVoidCast = ScalarExprEmitter::isNotVoidCast(CastSrcTy, DestTy);
  ...
  if (CGF->SanOpts.has(SanitizerKind::TypePlus) && llvm::ClCreateUnrelatedCastTypeList) {
    ...
    if (!CastSrcTy->isVoidPointerType() &&
        !DestTy->getPointeeType().isNull() &&
        !DestTy->getPointeeType()->isStructureOrClassType() &&
        CastSrcTy->isAnyPointerType() &&
        ((!CastSrcTy->getPointeeType().isNull() && CastSrcTy->getPointeeType()->isStructureOrClassType()) ||
         DestTy->getPointeeType()->isUnionType())) {
      if (llvm::ClVoidStar) {
        llvm::HexTypeCommonUtil::logClassToFile(
            ConvertType(CastSrcTy->getPointeeType()), "/cast_to_void");
      }
    } else if (!CastSrcTy->getPointeeType().isNull() && !CastSrcTy->getPointeeType()->isStructureOrClassType() &&
                   !DestTy->isVoidPointerType() &&
                   DestTy->isAnyPointerType() &&
                   (DestTy->getPointeeType()->isStructureOrClassType() ||
        (!DestTy->getPointeeType().isNull() &&
               DestTy->getPointeeType()->isUnionType()))) {
        llvm::HexTypeCommonUtil::logClassToFile(
            ConvertType(DestTy->getPointeeType()), "/cast_from_void");
    }
...
Value *ScalarExprEmitter::VisitCastExpr(CastExpr *CE) {
  ...
  case CK_BitCast: {
    ...
    logBitCastClasses(CE, &CGF);


    // VTable: To insert constructor calling instrumentation
    bool debug = false;
    if (CGF.SanOpts.has(SanitizerKind::TypePlus)  && llvm::ClVtableStandard && !llvm::ClNoSetSourcererRTTI) {
      if(llvm::CallBase* mallocCall = dyn_cast<llvm::CallBase>(Src)) {
        AllocatorType allocatorType = AllocatorType::NOT_ALLOCATOR;
        ...
        if (isa<CStyleCastExpr>(CE) || isa<CXXReinterpretCastExpr>(CE) || isa<CXXStaticCastExpr>(CE)) {
          ...
          allocatorType = res.first;
          custom_header_size = res.second;
        }

        bool in_allocator_with_header = CodeGenFunction::needsHeaderSpace(Builder.GetInsertBlock()->getParent()->getName().str(), "") > 0;
        ...
        if (allocatorType != AllocatorType::NOT_ALLOCATOR && DerivedClassDecl) {
          if (DerivedClassDecl->isUnion()){
            llvm::errs() << "union\n";
          ...
          if (double_ptr < 2) {
            ...
            if (!DerivedClassDecl->hasDefinition()){ // NICOLAS TODO check when this happends
            ...
            for (CXXMethodDecl *MD : llvm::make_range(DerivedClassDecl->method_begin(), DerivedClassDecl->method_end())) {
              if (VptrInsertorDecl *target = dyn_cast<VptrInsertorDecl>(MD)) {
                if (target->isDefaultConstructor()) {
                  if(llvm::CallBase* ci = dyn_cast<llvm::CallBase>(Src)) {
                    if (debug) {
                      ...
                      llvm::errs() << "Dest " << DestTy.getAsString() << " vs allocatedType " << allocatedType << "\n";
                      E->getExprLoc().dump(CGF.getContext().getSourceManager());
                    } 
                    ...
                    ArrayRef<Expr *> Args = llvm::ArrayRef(argsVec);
                    VptrInsertorExpr *cee = VptrInsertorExpr::Create(CGF.getContext(), DestTy->getPointeeType(), E->getExprLoc(),
                      target, false, Args,
                      false, false, false, false,
                      CXXConstructionKind::Complete, E->getSourceRange());
                    OffsetAndNbrObject pair = CGF.numberOfConstructorCall(mallocCall, allocatedType, allocatorType, custom_header_size);
                    Value* loopStartVal = pair.first;
                    Value* nbrObj = pair.second;
                    if(in_allocator_with_header) {
                      // In the custom allocator in Povray (pov_malloc), there is a call to malloc which will trigger 
                      // the vptr insertor for the header metadata on the whole allocated space instead of once. 
                      nbrObj = Builder.getInt32(1);
                    }
                    llvm::Type* realDstTy = ConvertType(DestTy->getPointeeType());
                    Value* addressWithNewType = Builder.CreateGEP(realDstTy, Builder.CreateBitCast(Src, DstTy), loopStartVal);
                    Address addr = Address(addressWithNewType, realDstTy, CGF.getContext().getTypeAlignInChars(DestTy));
                    CGF.EmitCXXAggrVptrInsertorCall(target, nbrObj, addr, cee, false, false, true);
                    // We could not validate the cast in this case.
                    // return Builder.CreateBitCast(Src, DstTy);
                    break;
                  }
                }
  ...
  case CK_ToVoid: {
    if (CGF.SanOpts.has(SanitizerKind::TypePlus) && llvm::ClCreateUnrelatedCastTypeList && llvm::ClVoidStar) {
      if(llvm::ClCastObjOpt) {
        llvm::errs() << "The option \"cast-obj-opt\" and \"create-cast-..._list\" cannot but used at the same time.\n";
        exit(1);
      }
      QualType CastSrcTy = E->getType();
      if(CastSrcTy->isAnyPointerType() && CastSrcTy->getPointeeType()->isStructureOrClassType()) {
        llvm::HexTypeCommonUtil::logClassToFile(ConvertType(CastSrcTy->getPointeeType()), "/cast_to_void");
      }
    }
    CGF.EmitIgnoredExpr(E);
    return nullptr;
