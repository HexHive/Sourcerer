...
private:
  ...
  CFGBlock *VisitVptrInsertorExpr(VptrInsertorExpr *C, AddStmtChoice asc);
  ...
  template <typename CallLikeExpr,
            typename = std::enable_if_t<
                ...
                std::is_base_of_v<VptrInsertorExpr, CallLikeExpr> ||
  ...
  void appendVptrInsertor(CFGBlock *B, VptrInsertorExpr *CE) {
    if (const ConstructionContext *CC =
            retrieveAndCleanupConstructionContext(CE)) {
      B->appendVptrInsertor(CE, CC, cfg->getBumpVectorContext());
      return;
    }
...
std::unique_ptr<CFG> CFGBuilder::buildCFG(const Decl *D, Stmt *Statement) {
  ...
  if (const auto *CD = dyn_cast_or_null<VptrInsertorDecl>(D)) {
    CFGBlock *VBaseSucc = nullptr;
    for (auto *I : llvm::reverse(CD->inits())) {
      if (BuildOpts.AddVirtualBaseBranches && !VBaseSucc &&
          I->isBaseInitializer() && I->isBaseVirtual()) {
...
CFGBlock *CFGBuilder::Visit(Stmt * S, AddStmtChoice asc,
  ...
  switch (S->getStmtClass()) {
    ...
    case Stmt::VptrInsertorExprClass:
      return VisitVptrInsertorExpr(cast<VptrInsertorExpr>(S), asc);
...
CFGBlock *CFGBuilder::VisitVptrInsertorExpr(VptrInsertorExpr *C,
                                            AddStmtChoice asc) {
