...
static bool HandleVptrInsertorCall(const Expr *E, const LValue &This,
                                  CallRef Call,
                                  const VptrInsertorDecl *Definition,
  ...
  if (Definition->isDelegatingConstructor()) {
    VptrInsertorDecl::init_const_iterator I = Definition->init_begin();
...
static bool HandleVptrInsertorCall(const Expr *E, const LValue &This,
  ...
  return HandleVptrInsertorCall(E, This, Call, Definition, Info, Result) &&
         CallScope.destroy();
}
static bool HandleConstructorCall(const Expr *E, const LValue &This,
                                  ArrayRef<const Expr*> Args,
...
namespace {
  ...
  public:
    ...
    bool VisitVptrInsertorExpr(const VptrInsertorExpr *E) {
      return VisitVptrInsertorExpr(E, E->getType());
    }
    bool VisitLambdaExpr(const LambdaExpr *E);
    bool VisitCXXInheritedCtorInitExpr(const CXXInheritedCtorInitExpr *E);
    bool VisitCXXConstructExpr(const CXXConstructExpr *E, QualType T);
    bool autox();
    bool VisitVptrInsertorExpr(const VptrInsertorExpr *E, QualType T);
...
bool RecordExprEvaluator::VisitVptrInsertorExpr(const VptrInsertorExpr *E,
                                                QualType T) {
  // Note that E's type is not necessarily the type of our class here; we might
  // be initializing an array element instead.
  const VptrInsertorDecl *FD = E->getConstructor();
  ...
  auto Args = llvm::ArrayRef(E->getArgs(), E->getNumArgs());
  return HandleVptrInsertorCall(E, This, Args,
                               cast<VptrInsertorDecl>(Definition), Info,
                               Result);
}
...
bool ArrayExprEvaluator::VisitVptrInsertorExpr(const VptrInsertorExpr *E) {
  return VisitVptrInsertorExpr(E, This, &Result, E->getType());
}
...
bool ArrayExprEvaluator::VisitVptrInsertorExpr(const VptrInsertorExpr *E,
  ...
  if (const ConstantArrayType *CAT = Info.Ctx.getAsConstantArrayType(Type)) {
    ...
    *Value = APValue(APValue::UninitArray(), N, N);
    ...
    for (unsigned I = 0; I != N; ++I)
      if (!VisitVptrInsertorExpr(E, ArrayElt, &Value->getArrayInitializedElt(I),
                                 CAT->getElementType()) ||
...
bool Expr::isPotentialConstantExpr(const FunctionDecl *FD,
  ...
  } else if (const VptrInsertorDecl *CD = dyn_cast<VptrInsertorDecl>(FD)) {
    // Evaluate the call as a constant initializer, to allow the construction
    // of objects of non-literal types.
    Info.setEvaluatingDecl(This.getLValueBase(), Scratch);
    HandleVptrInsertorCall(&VIE, This, Args, CD, Info, Scratch);
  } else {
