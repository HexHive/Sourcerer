...
int DeclarationName::compare(DeclarationName LHS, DeclarationName RHS) {
  ...
  case DeclarationName::CXXConstructorName:
    if (QualTypeOrdering()(LHS.getCXXNameType(), RHS.getCXXNameType()) || RHS.vptrInsertor != LHS.vptrInsertor)
      return -1;
...
void DeclarationName::print(raw_ostream &OS,
  ...
  case DeclarationName::CXXConstructorName:
    ...
    return printCXXConstructorDestructorName(getCXXNameType(), OS, Policy);
      ...
      printCXXConstructorDestructorName(getCXXNameType(), OS, Policy);
      OS << "_vptr_insertor";
      return;
    }
...
DeclarationName DeclarationNameTable::getVptrInsertorName(CanQualType Ty) {
  ...
  void *InsertPos = nullptr;
  if (auto *Name = CXXVptrInsertorNames.FindNodeOrInsertPos(ID, InsertPos)) {
    return {Name, DeclarationName::StoredVptrInsertorName, true};
  }

  // We have to create it.
  auto *SpecialName = new (Ctx) detail::CXXSpecialNameExtra(Ty);
  CXXVptrInsertorNames.InsertNode(SpecialName, InsertPos);
  return {SpecialName, DeclarationName::StoredVptrInsertorName, true};
}
