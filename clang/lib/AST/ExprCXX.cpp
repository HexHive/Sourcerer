...
SourceLocation VptrInsertorExpr::getBeginLoc() const {
  if (isa<CXXTemporaryObjectExpr>(this))
    return cast<CXXTemporaryObjectExpr>(this)->getBeginLoc();
  return getLocation();
}

SourceLocation VptrInsertorExpr::getEndLoc() const {
  if (isa<CXXTemporaryObjectExpr>(this))
...
VptrInsertorExpr *VptrInsertorExpr::Create(
    const ASTContext &Ctx, QualType Ty, SourceLocation Loc,
    VptrInsertorDecl *Ctor, bool Elidable, ArrayRef<Expr *> Args,
    bool HadMultipleCandidates, bool ListInitialization,
    bool StdInitListInitialization, bool ZeroInitialization,
    CXXConstructionKind ConstructKind, SourceRange ParenOrBraceRange) {
  void *Mem = Ctx.Allocate(sizeof(VptrInsertorExpr),
                           alignof(VptrInsertorExpr));
  return new (Mem) VptrInsertorExpr(
      VptrInsertorExprClass, Ty, Loc, Ctor, Elidable, Args,
      HadMultipleCandidates, ListInitialization, StdInitListInitialization,
      ZeroInitialization, ConstructKind, ParenOrBraceRange);
}

VptrInsertorExpr *VptrInsertorExpr::CreateEmpty(const ASTContext &Ctx,
                                                unsigned NumArgs) {
  unsigned SizeOfTrailingObjects = sizeOfTrailingObjects(NumArgs);
  void *Mem = Ctx.Allocate(sizeof(VptrInsertorExpr),
                           alignof(VptrInsertorExpr));
  return new (Mem)
      VptrInsertorExpr(CXXConstructExprClass, EmptyShell(), NumArgs);
}

VptrInsertorExpr::VptrInsertorExpr(
    StmtClass SC, QualType Ty, SourceLocation Loc, VptrInsertorDecl *Ctor,
    bool Elidable, ArrayRef<Expr *> Args, bool HadMultipleCandidates,
    bool ListInitialization, bool StdInitListInitialization,
    bool ZeroInitialization, CXXConstructionKind ConstructKind,
    SourceRange ParenOrBraceRange)
    : Expr(SC, Ty, VK_PRValue, OK_Ordinary), Constructor(Ctor),
      NumArgs(Args.size()), ParenOrBraceRange(ParenOrBraceRange) {
  VptrInsertorExprBits.Elidable = Elidable;
  VptrInsertorExprBits.HadMultipleCandidates = HadMultipleCandidates;
  VptrInsertorExprBits.ListInitialization = ListInitialization;
  VptrInsertorExprBits.StdInitListInitialization = StdInitListInitialization;
  VptrInsertorExprBits.ZeroInitialization = ZeroInitialization;
  VptrInsertorExprBits.ConstructionKind = llvm::to_underlying(ConstructKind);
  VptrInsertorExprBits.Loc = Loc;
...
VptrInsertorExpr::VptrInsertorExpr(StmtClass SC, EmptyShell Empty,
                                   unsigned NumArgs)
    : Expr(SC, Empty), NumArgs(NumArgs) {}
